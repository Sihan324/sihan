...
The current model choices are summarised below. The table separates the preferred reporting model from the main reason for selection, because the best model depends on whether the goal is in-sample fit, residual adequacy, or forecast accuracy.

| Series | Preferred model | Main basis for selection | Diagnostic caveat | Interpretation |
|---|---|---|---|---|
| TFR | ARIMA with drift | Lowest ten-year holdout RMSE, 0.0281 | AICc is slightly worse than `arima_auto` | Best current short-horizon forecasting option for fertility |
| Log total live-births | ARIMA errors with TFR regressor | Lowest AICc, -192.53, and lowest conditional holdout RMSE, 0.123 | Ljung-Box p-value 0.0185 indicates residual autocorrelation | Useful explanatory and conditional forecasting model, but not fully adequate |
